...
export const App = () => {
  ...
  const [contacts, setContacts] = useState(JSON.parse(localStorage.getItem(KEY_STORAGE))
 ?? [
  ...
  const deleteContact = (id) => {
    const remainedContacts = contacts.filter(contact => contact.id !== id);
  ...
  return (
    <>
      <Form getFormData={addContact}></Form>
      <Filter handleFilterChange={filterContacts} value={filter}></Filter>
      <Contacts listContacts={filteredContacts} onDeleteClick={deleteContact}></Contacts>      
</>
  )
}
